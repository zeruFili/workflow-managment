...
export function QuantitySurveyorTasks() {
  ...
  const fetchTasks = useCallback(async (page: number, search: string, status: string, force = false): Promise<QuantitySurveyorTaskItem[] | undefined> => {
    if (!user) return;
  ...
  const handlePageChange = (page: number) => {
    quantitySurveyorTaskCache.invalidate({ page: apiPage, limit: PAGE_SIZE, ...(searchTerm ? { search: searchTerm } : {}), ...(statusFilter ? { status: statusFilter } : {}) });
    setApiPage(page);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-col md:flex-row">
        <div>
          ...
          {canSubmit && counts.quantitySurveyorTasks > 0 && (
            <div className="mt-3 bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <p className="text-sm text-blue-700 font-medium">
                {counts.quantitySurveyorTasks} unseen {counts.quantitySurveyorTasks === 1 ? 'notification' : 'notifications'} awaiting your attention
              </p>
            </div>
          )}
        ...
        <div className="card-safe overflow-hidden min-w-0 bg-white rounded-xl p-12 shadow-sm border border-gray-200 text-center">
          ...
          <p className="text-gray-500">
            {searchTerm
              ...
              : 'No quantity surveying tasks yet.'}
          </p>
        </div>
      ) : (
        <>
